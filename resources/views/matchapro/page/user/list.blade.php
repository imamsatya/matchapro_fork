        ...
        <div class="card">
            <div class="card-header border-bottom p-1">
                ...
                <div class="dt-action-buttons text-end">
                    <div class="dt-button d-inline-flex">
                        <a href="javascript:void(0)">
                            <button class="dt-button btn btn-primary waves-effect waves-light" id="add-new-user"><i data-feather="plus"
                                    class="me-25"></i>
            ...
            <div class="card-body">                
                <div class="table-responsive container-table-user">
                    <table class="table table-hover table-bordered" id="users-table">
                        <thead>
                            <tr>                                
                                <th>Nama</th>
                                <th>Satuan Kerja</th>
                                <th>Hak Akses Wilayah</th>
                                <th>Role</th>
                                <th>Status</th>
                                <th>Actions</th>
                            </tr>
        ...
        <div class="modal-content">
            <div class="modal-header bg-light-secondary">
                <h5 class="modal-title text-secondary" id="editWilayahModalLabel">
                    <i data-feather="edit" class="me-1"></i>Edit User - Akses Wilayah                    
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
...
<div class="modal fade" id="addUserModal" tabindex="-1" aria-labelledby="addUserModalLabel" aria-hidden="true" data-bs-backdrop="static">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            <div class="modal-header bg-light-secondary">
                <h5 class="modal-title text-secondary" id="addUserModalLabel">
                    <i data-feather="user-plus" class="me-1"></i>Add New User
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body container-modal-add-user">
                <form id="addUserForm" class="row gy-2">
                    <div class="col-md-6">
                        ...
                        <div class="invalid-feedback"><span id="add-name-error"></span></div>
                    </div>
                    <div class="col-md-6">
                        <label for="add-username" class="form-label">Username <span class="text-danger">*</span></label>
                        <input type="text" class="form-control" id="add-username" name="add-username" required>
                    ...
                    <div class="col-md-6">
                        <label class="form-label" for="add-whatsapp">Nomor Whatsapp</label>
                        <input type="text" class="form-control" id="add-whatsapp" placeholder="Nomor Whatsapp" name="add-whatsapp"
                            aria-label="Nomor Whatsapp">
                    </div>                    
                </form>
            </div>
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-relief-primary" id="saveAddUser">
                    <i data-feather="save" class="me-1"></i>Save changes
                </button>
            </div>
        </div>
...
<script src="{{ asset(mix('vendors/js/forms/select/select2.full.min.js')) }}"></script>
    <script>
        $(document).ready(function() {
            ...
            $('#add-provinsi, #add-kabupaten').select2({
            ...
            var table_users = $('#users-table').DataTable({
                ...
                ajax: {
                    ...
                    data: function(d) {
                        ...
                        d.assign_wilayah = $('select[name="assign_wilayah"]').val();
                    }                 
                },
                columns: [                    
                    { data: 'name', name: 'name' , 
                    ...
                    { data: 'satuan_kerja', name: 'satuan_kerja', width: '20%',
                        render: function(data, type, full, meta) {
                            // kalau belum ada role
                            if(!full.role) {return `<span class="text-muted">-</span>`;}
                            else if(full.role && data == '0000') {
                                return `PUSAT<br/><span class="text-muted">${data}</span>`;
                            } else if(full.role && full.kode_kabupaten == '00') {
                                return `PROVINSI ${full.nama_provinsi}<br/><span class="text-muted">${data}</span>`;
                            } else {                                
                                return `${full.kode_kabupaten >= 70 ? 'KOTA' : 'KABUPATEN'} ${full.nama_kabupaten}<br/><span class="text-muted">${data}</span>`;
                            }
                        }
                     },
                    {
                        data: 'akses_wilayah', name: 'akses_wilayah', width: '10%',
                        render: function(data, type, full, meta) {                                                      
                            if(!data.length && !full.role) return `<span class="badge bg-light-danger">Belum Di-assign</span>`;                        
                            if(!data.length && full.role && !full.role.includes('PUSAT')) return `<span class="badge bg-light-danger">Belum Di-assign</span>`;                        
                            if(full.role.includes('PUSAT')) return `<span class="badge bg-light-info">Seluruh Indonesia</span>`                            
            ...
            table_users.on('preXhr.dt', function(){
                blockProgress($('.container-table-user'));
            })

            table_users.on('xhr.dt', function(){
                unblockProgress($('.container-table-user'));
            })
            ...
            $('#users-table thead tr:eq(1) th').each(function(i) {
                if(i < 2) {
                    ...
                    $(this).html(`<select name="assign_wilayah" class="form-control form-control-sm">
            ...
            $("#users-table tbody").on('click', '.delete-user', function() {
                ...
                Swal.fire({
                    title: 'Warning!',
                    html: `Apakah yakin akan menghapus user?
                        <p class="text-danger mb-0 fw-bold">${rowData.name}</p>
                        `,
                    icon: 'warning',
                    ...
                    if (result.isConfirmed) {
                        let id_user = rowData.id;
                        blockProgress('body');
                        $.ajax({
                            url: "{{ route('user.delete') }}",
                            type: 'POST',
                            dataType: 'json',
                            data: { user: id_user, _token: '{{ csrf_token() }}' },
            ...
            function sendDataAdd() {
                blockProgress($('.container-modal-add-user'));
                $.ajax({
                    url: '{{ route("user.add") }}',
                    type: 'POST',
                    data: {                        
                    ...
                    dataType: 'json',
                    success: function(response) {
                        unblockProgress($('.container-modal-add-user'));
                        Swal.fire({
                            title: 'Success!',
            ...
            $("#addUserModal").on('shown.bs.modal', function() {
                setOption($("#add-provinsi"), optionProv());                
            });

            $("#add-provinsi").on('change', function(){
                let provId = $(this).val();
                if(!provId) {
                    setOption($("#add-kabupaten"), optionKab([]));                                    
                    return;
                }

                if(cacheKab.length && cacheKab.find(k => k.provinsi_id == provId)) {
                    let m_kab = cacheKab.find(k => k.provinsi_id == provId).kabupaten;
                    setOption($("#add-kabupaten"), optionKab(m_kab));                    
                    return;
                }
                ...
                getWilayahKab(provId).then(function(response) {
                    setOption($("#add-kabupaten"), optionKab(response));
                    $("#add-kabupaten").val('').trigger('change');
                    unblockProgress($('.container-modal-add-user'));
                }).catch(function(err) {
                    unblockProgress($('.container-modal-add-user'));
                    Swal.fire({
            ...
            function initializeFormAdd(){
                formDataAdd = {
                    name: $("#add-name").val().trim(),
                    username: $("#add-username").val().trim(),
                    provinsi_id: $("#add-provinsi").val(),
                    kabupaten_kota_id: $("#add-kabupaten").val(),
                    role_id: $("#add-role").val(),                    
                    whatsapp: $("#add-whatsapp").val()
                } 
            }
            ...
            function renderButton(data) {
                return `
                <div class="d-flex align-items-center col-actions">
                    <a data-bs-toggle="tooltip" data-bs-placement="top" title="Edit User Info" href="javascript:void(0);" class="btn btn-sm btn-flat-warning me-0 edit-user" data-user-id="${data.id}">${feather.icons['user'].toSvg({ class: 'font-small-4' })}</a> 
                    <a data-bs-toggle="tooltip" data-bs-placement="top" title="Edit Hak Akses Wilayah" href="javascript:void(0);" class="btn btn-sm btn-flat-primary me-0 edit-user-wilayah" data-user-id="${data.id}">${feather.icons['edit'].toSvg({ class: 'font-small-4' })}</a> 
                    <a data-bs-toggle="tooltip" data-bs-placement="top" title="Delete" href="javascript:void(0);" class="btn btn-sm btn-flat-danger me-0 delete-user" data-user-id="${data.id}">${feather.icons['trash'].toSvg({ class: 'font-small-4' })}</a> 
                ...
                return `
                    <div class="d-flex justify-content-left align-items-center">
                        ...
                        <div class="d-flex flex-column">
                            ...
                            <span class="fw-bolder">${data.name ? data.name : '-'}</span>
                            </a>
